...
open(FILE,'prof_course_data.xml');
my $xml = <FILE>;
close(FILE);
...
my $insert_query = "insert into test.professorcourses (id, name, email, profile, semestertype, semesteryear, coursenumber, coursetitle, courselink) values values(?,?,?,?,?,?,?,?,?)";
my $sth = $dbh->prepare( $insert_query );
print Dumper(@profarray);
